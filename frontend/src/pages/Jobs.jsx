...
import { listJobs, createJob, deleteJob } from "../services/api";
...
export default function Jobs() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title: "", company: "", description: "", required_skills: "", preferred_skills: "", min_experience_years: 0, education_level: "bachelors", education_field: "" });
      setShowForm(false);
      fetchJobs();
    } catch (err) {
  ...
  const handleDelete = async (jobId) => {
    if (!window.confirm("Delete this job and all its resumes?")) return;
    try {
    ...
    <div>
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 20 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 16 }}>Create New Job Posting</h3>
